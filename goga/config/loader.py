...
def _parse_language(data: dict) -> str:
    ...
    try:
        ...
        raise KeyError("language is required in .goga.yml") from err
...
def load_config() -> Config:
    """Load project configuration from .goga.yml in the current working directory.

    Returns:
        Config instance with parsed BuildConfig and TaskExecutor.

    Raises:
        FileNotFoundError: if .goga.yml does not exist or is empty.
        ValueError: if .goga.yml is not a YAML mapping or invalid field values.
        KeyError: if required sections are missing (language, build, build.task_executor).
        yaml.YAMLError: if YAML parsing fails.
    """
    config_path = Path("./.goga.yml")

    if not config_path.exists():
        raise FileNotFoundError(".goga.yml not found in project root")
    ...
    if not isinstance(data, dict):
        raise ValueError(".goga.yml must be a YAML mapping")
    ...
    try:
        ...
        raise KeyError("build is required in .goga.yml") from err

    if not isinstance(build_data, dict):
        raise ValueError("'build' must be a mapping in .goga.yml")

    if not isinstance(commands, dict):
        raise ValueError("'commands' must be a mapping in .goga.yml")
    commands = dict(commands)

    try:
        task_executor_data = build_data["task_executor"]
    except KeyError as err:
        raise KeyError("build.task_executor is required in .goga.yml") from err

    if not isinstance(task_executor_data, dict):
        raise ValueError("build.task_executor must be a mapping in .goga.yml")
