...
- **Optional**: task file at the path printed by `goga history path -f task.md` — when present, used to verify requirements coverage
...
If the task file at the path printed by `goga history path -f task.md` exists:
