...
Verify the execution plan (`.goga/history/<year>/<topic>/plan.md`) for **completeness and correctness** against the design document and `CODEMANIFEST` contracts before passing to ralphex.
...
- Plan file at `.goga/history/<year>/<topic>/plan.md` — the execution plan, verified against sources of truth
...
- **Apply suggested fix**: update the plan file at `.goga/history/<year>/<topic>/plan.md`, then re-verify that the fix introduces no new issues (re-run the relevant checks). Briefly report the re-verification result.
