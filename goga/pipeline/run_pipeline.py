...
def run_pipeline(name: str, project_dir: Path, user_dir: Path) -> int:
    """Run a goga pipeline by name via the external ``flowmanager`` binary.
    ...
    ``flowmanager`` via :func:`goga.afm.run_flow` with the absolute path. The
    ``flowmanager`` binary's exit code is propagated; a missing pipeline
    returns a non-zero code without invoking the binary.
    ...
    Returns:
        ``0`` on success; ``1`` when the named pipeline is not found; ``127``
        when the ``flowmanager`` binary is missing from ``PATH``; ``126`` when
        the binary cannot be invoked; otherwise the ``flowmanager`` exit code.
    """
    entries = list_pipelines(project_dir, user_dir)
    match = next((entry for entry in entries if entry.name == name), None)

    if match is None:
        print(f"Error: pipeline '{name}' not found", file=sys.stderr)
        return 1
