...
class ReviewOptions:
    ...
    `two_pass` is True when a review executor agent is set and differs from the
    task executor agent. Branch priority between `skip` and `two_pass` belongs
    to the orchestrator, not to this value-object.
    """
...
def resolve_review_options(config: BuildConfig, cli_options: dict) -> ReviewOptions:
    ...
    two_pass = review_agent is not None and review_agent != config.task_executor.agent

    return ReviewOptions(skip=skip, review_agent=review_agent, roles=roles, two_pass=two_pass)
