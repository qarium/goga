...
| `->Re-exports`           | Задача: обеспечить импортируемость из фасада (согласно `goga-lang-disp`) |
...
Конкретные команды берутся из `goga-lang-disp` (через `goga-lang-disp`).

```markdown
## Команды валидации
- <команда запуска всех тестов из `goga-lang-disp`>: Запустить все тесты
- <команда линта из `goga-lang-disp`>: Проверка линта
- <команда проверки фасада из `goga-lang-disp`>: Доступность фасада
```
