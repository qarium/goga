...
- Обязанность фасада: должен быть импортируем из фасада (согласно скиллу `goga-lang-disp`)
...
<Абзац контекста: что делает эта задача — структура Cell, фасад (согласно скиллу `goga-lang-disp`), реэкспорты. Предоставить достаточно контекста для ИИ-агента, чтобы реализовать эту задачу независимо.>
...
- [ ] <конкретный шаг реализации 2 — например, "Добавить реэкспорт `EntityName` в фасад (согласно скиллу `goga-lang-disp`)">
- [ ] Проверить доступность фасада: <команда проверки фасада из скилла `goga-lang-disp`>
- [ ] Линт: <команда линта из скилла `goga-lang-disp`> — исправить форматирование при необходимости
...
- [ ] **Верификация интерфейсов**: запустить контрактные тесты выше — <команда запуска тестов сущности из скилла `goga-lang-disp`> — все должны пройти
- [ ] **Логические тесты**: <конкретные поведенческие тесты — позитивные, негативные, граничные случаи из плана>
- [ ] **Отладка**: <команда запуска всех тестов из скилла `goga-lang-disp`> — исправлять код реализации, пока все тесты не пройдут (НЕ исправлять тестовый код)
- [ ] **Перепроверка контрактов**: проверить все обязательства контракта — фасад, форма API, поведение
- [ ] **Линт**: <команда линта из скилла `goga-lang-disp`> — исправить форматирование, применить декомпозицию при необходимости
...
- [ ] Создать файл тестов <файл тестов — из скилла `goga-lang-disp`>
...
- [ ] Запустить валидацию: <команда запуска тестов из скилла `goga-lang-disp`>
...
- `<команда>`: <что она проверяет>
- <команда запуска всех тестов из `goga-lang-disp`>: Запустить все тесты
- <команда проверки фасада из `goga-lang-disp`>: Проверить, что все сущности фасада импортируемы
