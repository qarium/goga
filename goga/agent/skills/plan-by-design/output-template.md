...
- Обязанность фасада: должен быть импортируем из фасада (согласно скиллу `goga-lang-helper`)
...
<Абзац контекста: что делает эта задача — структура Cell, фасад (согласно скиллу `goga-lang-helper`), реэкспорты. Предоставить достаточно контекста для ИИ-агента, чтобы реализовать эту задачу независимо.>
...
- [ ] <конкретный шаг реализации 2 — например, "Добавить реэкспорт `EntityName` в фасад (согласно скиллу `goga-lang-helper`)">
- [ ] Проверить доступность фасада: <команда проверки фасада из скилла `goga-lang-helper`>
- [ ] Линт: <команда линта из скилла `goga-lang-helper`> — исправить форматирование при необходимости
...
- [ ] **Верификация интерфейсов**: запустить контрактные тесты выше — <команда запуска тестов сущности из скилла `goga-lang-helper`> — все должны пройти
- [ ] **Логические тесты**: <конкретные поведенческие тесты — позитивные, негативные, граничные случаи из плана>
- [ ] **Отладка**: <команда запуска всех тестов из скилла `goga-lang-helper`> — исправлять код реализации, пока все тесты не пройдут (НЕ исправлять тестовый код)
- [ ] **Перепроверка контрактов**: проверить все обязательства контракта — фасад, форма API, поведение
- [ ] **Линт**: <команда линта из скилла `goga-lang-helper`> — исправить форматирование, применить декомпозицию при необходимости
...
- [ ] Создать файл тестов <файл тестов — из скилла `goga-lang-helper`>
...
- [ ] Запустить валидацию: <команда запуска тестов из скилла `goga-lang-helper`>
...
- <команда проверки фасада из `goga-lang-helper`>: Проверить, что все сущности фасада импортируемы
