...
### usage файлы внутри .usages/ — два уровня

Практики живут на двух уровнях:

**Проектный `.usages/`** (в корне проекта) — общие практики не привязанные к конкретной cell: библиотеки, инструменты,
...
- Если практика описывает библиотеку/инструмент/конвенцию общую для проекта — проектный `.usages/`
...
- Имя файла должно быть кратким, осмысленным и совпадать с ключом, под которым практика подключается в директиве
  `Usages` в `CODEMANIFEST`
...
**Файл** — путь к md файлу (относительно рабочего каталога проекта):
- Практика объёмная и засорит CODEMANIFEST если написать inline
- Практика переиспользуется в нескольких cells — файл живёт в проектном `.usages/` и подключается через путь к md файлу
