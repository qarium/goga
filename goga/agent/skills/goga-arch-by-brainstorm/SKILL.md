...
12. **Embedding из Imports** — каждый встраиваемый тип (через `->`) доступен через `Imports`
13. **Мутации из доступных типов** — базовые типы в мутациях (`Object::Target`) доступны: импортированы через `Imports`
    или объявлены в текущем CODEMANIFEST
14. **Entity / Routine корректность** — каждый тип с `methods` и/или `properties` является Entity; каждый тип без
    `methods` и `properties` является Routine и не содержит этих секций
15. **Базовые usages из конфигурации** — если скилл `goga-codemanifest-base` вернул базовые usages, каждая
    базовая практика включена в директиву `Usages` всех CODEMANIFEST и на неё есть ссылка хотя бы в одной аннотации
16. **Базовые annotations из конфигурации** — если скилл `goga-codemanifest-base` вернул базовые annotations, базовые
