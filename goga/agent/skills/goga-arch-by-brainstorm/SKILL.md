...
1. **Предложите CODEMANIFEST** — Header (Imports, Usages, Annotations), Body (мутации, сигнатура, методы, свойства, аннотации),
   ...
   При проектировании CODEMANIFEST учитывайте базовые annotations и usages из Фазы 3, Шаг 2:
   - **Базовые usages** — впишите в директиву `Usages` заголовка CODEMANIFEST базовые usages.
     Если базовая практика влияет на форму контракта (например, требует определённую обработку ошибок,
     конвенцию именования, паттерн) — контракт должен ей соответствовать. Ссылайтесь на релевантные базовые usages
     в annotations через `` `usage_name` ``
   - **Базовые annotations** — впишите в директиву `Annotations` заголовка CODEMANIFEST базовые annotations проекта.
     Содержимое базовых annotations должно учитываться при проектировании контракта:
