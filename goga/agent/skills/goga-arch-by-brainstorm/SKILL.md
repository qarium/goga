...
- **Из файла задачи** — путь к файлу `docs/tasks/<topic>.md`, сформированному скиллом `goga-task-by-grooming`
...
Если задача подразумевает использование внешних библиотек/технологий — прочитайте usages о них (в `.usages/tools/`),
...
1. **Определите usages** — какие практики нужны типам в этой cell:
   ...
   - **Usages внешних библиотек** — если типы в этой cell используют внешнюю библиотеку,
     для которой есть usage-файл в проекте (`.usages/tools/`), подключите его через директиву `Usages`.
     Если usage-файла для библиотеки нет — зафиксируйте необходимость его создания
     (создание выполняется в отдельном скилле)
