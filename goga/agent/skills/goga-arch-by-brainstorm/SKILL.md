...
**Цель:** Построить «оглавление» всех типов — только имена, краткое назначение и связи между ними.
Без методов, свойств и деталей — только каркас доменной модели.

На основе первичного анализа (Шаг 1) итеративно стройте карту типов:

1. **Предложите гипотезу карты типов** — список типов с кратким описанием и связями:
   ```
   DocumentRoot      — корень документа. Ссылается на: HeaderNode, BodyNode
   HeaderNode        — заголовок документа. Ссылается на: ImportNode
   ImportNode        — импорт. Ссылается на: DocumentRoot (resolve)
   ASTRule           — правило проверки. Принимает: DocumentRoot. Возвращает: ASTRuleError
   ASTRuleError      — ошибка правила. Ссылается на: ASTRule, DocumentNode
   ```
...
1. **Предложите детализацию** — методы, свойства, сигнатуры с типами параметров и возвращаемых значений:
   ...
   DocumentRoot
     ...
     properties:
       ...
       body -> BodyNode
     methods:
       validate() -> list[ASTRuleError]
       find_imports() -> list[ImportNode]

   ASTRule
     methods:
       check(doc: DocumentRoot) -> list[ASTRuleError]
...
#### Шаг 4. Разбиение на cells

**Цель:** Сгруппировать утверждённые типы (Шаг 3) в cells и определить границы клеток.

Предложите разбиение, основываясь на связности типов и зонах ответственности:

1. **Предложите гипотезу разбиения** — какие типы в какую cell попадают:
   ```
   cell: goga/ast/nodes
...
3. **Задайте один вопрос** — согласовано ли разбиение, нет ли типов, которые лучше перегруппировать

4. **Дождитесь обратной связи** — пользователь утверждает или просит изменения
   - **Утверждено** → перейдите к Шагу 5
   - **Не утверждено** → запомните замечания, вернитесь к пункту 1 и предложите скорректированное разбиение с учётом обратной связи.
...
- Пользователь утвердил разбиение

#### Шаг 5. Проектирование usages и annotations

**Цель:** Для каждой cell из разбиения (Шаг 4) определить, какие practices (usages) и инструкции
...
**Цель:** На основе утверждённых детализированных типов (Шаг 3), разбиения (Шаг 4)
...
1. **Соберите CODEMANIFEST** — «упакуйте» утверждённые типы в DSL-формат:
   ...
   Imports выводятся автоматически из разбиения (Шаг 4): если тип cell B определён в cell A,
   cell B добавляет его в Imports.Types из cell A.
