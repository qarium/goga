...
стеком, зависимостями и оценкой объёма. Результат становится входом для `goga-arch-by-brainstorm`.
...
Используйте для:
- Понимания двухуровневой модели usages (проектный `.usages/` и cell-level `<cell_path>/.usages/`)
- Принципов написания usage файлов при создании `.usages/tools/`
...
Выполните команду `goga config codemanifest.usages` чтобы получить базовые usages проекта.
Выполните команду `goga config codemanifest.annotations` чтобы получить базовые annotations.

Если команды вернули ошибку «Option not found» — базовые usages/annotations не заданы, зафиксируйте это как факт.

Если секция `codemanifest` существует:
1. **Прочитайте файлы практик** — для каждого usage прочитайте соответствующий md файл
2. **Проанализируйте базовые annotations** — зафиксируйте обязательные условия
...
1. **Определите стек реализации** — технологии, которые будут использованы:
   ...
   - Какие существующие usages проекта уже описывают используемые инструменты
...
2. **Определите внешние зависимости** — нужны ли компоненты, которых ещё нет в проекте:
   ...
   - Для каждой внешней зависимости определите, есть ли уже usage-файл в проекте (`.usages/tools/`)
...
- Определено, какие `.usages/tools` файлы нужно создать или обновить

### Фаза 5: Работа с .usages/tools
...
2. **Дождитесь обратной связи** — пользователь утверждает или просит изменения
   - **Утверждено** → создайте файл `.usages/tools/<name>.md`
...
2. **Дождитесь обратной связи** — пользователь утверждает или просит изменения
   - **Утверждено** → обновите файл `.usages/tools/<name>.md`
...
**Цель:** Сохранить сформулированную задачу в файл `docs/tasks/<topic>.md`.
...
|-----------|-----------|--------|
| <name> | `.usages/tools/<name>.md` | создан / обновлён / существует |
...
Сохраните файл и представьте пользователю сводку:
- Название задачи
- Стек
...
После утверждения сводки предложите пользователю следующий шаг — перейти к архитектуре задачи
с помощью `goga-arch-by-brainstorm`.
