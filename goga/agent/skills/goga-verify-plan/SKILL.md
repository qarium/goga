...
1. Используйте **Skill tool** для вызова `goga-lang-disp` — получите языковые правила реализации (фасад, реэкспорты, команды валидации)
...
Инфраструктурные задачи (структура Cell, фасад (согласно `goga-lang-disp`), реэкспорты) освобождены — они следуют упрощённому рабочему процессу.
...
1. **Инфраструктурные задачи** (структура Cell, фасад (согласно `goga-lang-disp`), реэкспорты)
...
### Фаза 11: Исправление (с одобрения пользователя)
