...
**Всегда предлагайте варианты ответа.** При обращении к пользователю за решением или подтверждением — всегда
предоставляйте конкретные варианта ответа. Никогда не задавайте открытые вопросы без предлагаемых для выбора
вариантов.
...
4. Получите текущую схему проекта:
    - Выполните `docker run --rm -v .:/project -w /project qarium/goga:latest schema`, чтобы получить иерархию
      существующих cells
...
7. Если существует файл задачи `docs/tasks/<topic>.md` — прочитайте его для последующей сверки покрытия требований
