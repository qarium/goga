...
**Оба существуют** — используйте **Skill tool** для вызова `goga-verify-plan` с именем функции в качестве аргумента.

ВАЖНО: Ваше единственное действие после подтверждения существования обоих документов — вызвать скилл `goga-verify-plan`. НЕ читайте файлы CODEMANIFEST, НЕ исследуйте кодовую базу, НЕ проверяйте план самостоятельно.
