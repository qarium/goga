...
board, and the orchestrators that bring the repository onto the chosen
host branch — purely by switching, or by creating the fresh work when
nothing hosts the identifier. Topic identity and statuses belong to the
...
def _switch_to_candidate(candidates: list[SwitchCandidate]) -> str:
    ...
    tail of ``switch_topic`` and ``ensure_topic``.
