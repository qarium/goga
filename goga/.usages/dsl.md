...
`Usages` определяет практики — документацию для потребителя API клетки.
...
1. **Объявление** в разделе `Usages` — практика описывается значением (путь к файлу или inline-текст)
2. **Импорт** через раздел `Imports` — практика импортируется из другой клетки по имени файла без расширения `.md`,
   полный путь файла — `{From}/.usages/{name}.md`. Импорт создаёт отслеживаемую зависимость, но не контрактное обязательство.
