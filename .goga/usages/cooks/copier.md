...
- `data` — словарь предзаполненных ответов шаблона; объединяется с дефолтами из `copier.yml` шаблона. `project_name` берётся из `resolve_project_name()` (общий дом `goga/config`) с fallback на click-prompt, когда имя не резолвится.
