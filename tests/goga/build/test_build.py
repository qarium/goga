...
class TestRunPrecondition:
    def test_claude_agent_succeeds(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
    ...
    def test_unsupported_agent_returns_1(self, tmp_path: Path) -> None:
...
class TestCreateClaudeSettings:
    ...
    def test_empty_env_creates_empty_section(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
            config = _make_config()
    ...
    def test_merge_preserves_existing_fields(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
...
class TestCreateClaudeWrapper:
    def test_wrapper_created(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
            config = _make_config()
    ...
    def test_config_defaults_added(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
            config = _make_config()
    ...
    def test_codex_enabled_true(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
    ...
    def test_codex_overwrites_existing(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
    ...
    def test_preserves_comments_and_custom_keys(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
            ralphex_dir = tmp_path / ".ralphex"
...
class TestCopyDefaults:
    def test_prompts_copied(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
            config = _make_config()
    ...
    def test_agents_copied(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
            config = _make_config()
    ...
    def test_defaults_dir_not_found_returns_1(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
            config = _make_config()
    ...
    def test_empty_defaults_subdirs_no_error(self, tmp_path: Path) -> None:
        original_cwd = str(Path.cwd())
        try:
            os.chdir(tmp_path)
...
class TestBuildDryRun:
    def test_dry_run_returns_0(self, tmp_path: Path) -> None:
    ...
    def test_dry_run_does_not_call_subprocess(self, tmp_path: Path) -> None:
...
class TestBuildFullExecution:
    ...
    def test_full_execution_returns_0(self, mock_which, mock_call, tmp_path: Path) -> None:
        result = _run_build_in_tmp(tmp_path, cli_options={"skip_manifest_check": True})
        assert result == 0

    @mock.patch.object(subprocess, "call", return_value=42)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_propagates_exit_code(self, mock_which, mock_call, tmp_path: Path) -> None:
        result = _run_build_in_tmp(tmp_path, cli_options={"skip_manifest_check": True})
        assert result == 42
...
class TestBuildEnvVarsInSettings:
    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_env_vars_in_settings_json(self, mock_which, mock_call, tmp_path: Path) -> None:
        config = _make_config(env=TEST_ENV_VARS)
        result = _run_build_in_tmp(
            tmp_path,
            config=config,
...
class TestBuildMissingRalphex:
    def test_ralphex_not_found_returns_1(self, tmp_path: Path) -> None:
...
class TestBuildDefaultsDirNotFound:
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    @mock.patch.object(subprocess, "call", return_value=0)
    def test_defaults_missing_returns_1(self, mock_call, mock_which, tmp_path: Path) -> None:
        with mock.patch("goga.build.build.DEFAULTS_PACKAGE_DIR", Path("/nonexistent")):
            result = _run_build_in_tmp(
                tmp_path,
                cli_options={"skip_manifest_check": True},
...
class TestBuildRepeatedBuild:
    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_repeated_build_overwrites(self, mock_which, mock_call, tmp_path: Path) -> None:
        cli_options = {"skip_manifest_check": True}
        _run_build_in_tmp(tmp_path, cli_options=cli_options)
...
class TestManifestCheck:
    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_all_committed_proceeds(self, mock_which, mock_call, tmp_path: Path) -> None:
        ...
        result = _run_build_in_tmp(tmp_path, cli_options={})
        assert result == 0

    def test_uncommitted_manifest_returns_1(self, tmp_path: Path) -> None:
        _init_git_repo(tmp_path)
        manifest = tmp_path / "CODEMANIFEST"
        manifest.write_text("content")
    ...
    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_skip_manifest_check(self, mock_which, mock_call, tmp_path: Path) -> None:
        _init_git_repo(tmp_path)
        manifest = tmp_path / "CODEMANIFEST"
        manifest.write_text("content")
    ...
    def test_not_git_repo_returns_1(self, tmp_path: Path) -> None:
        result = _run_build_in_tmp(tmp_path, cli_options={})
        assert result == 1

    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_no_codemanifest_files_proceeds(self, mock_which, mock_call, tmp_path: Path) -> None:
        _init_git_repo(tmp_path)
    ...
    def test_multiple_uncommitted_lists_all(self, tmp_path: Path) -> None:
        _init_git_repo(tmp_path)
        (tmp_path / ".gitkeep").write_text("")
        subprocess.run(["git", "add", ".gitkeep"], cwd=tmp_path, capture_output=True, check=True)
        subprocess.run(["git", "commit", "-m", "init"], cwd=tmp_path, capture_output=True, check=True)
...
class TestBuildNegativeCases:
    def test_invalid_settings_json_returns_nonzero(self, tmp_path: Path) -> None:
        claude_dir = tmp_path / ".claude"
        claude_dir.mkdir()
        (claude_dir / "settings.json").write_text("{broken json")
...
class TestBuildConfigFlags:
    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_worktree_from_config(self, mock_which, mock_call, tmp_path: Path) -> None:
        config = _make_config(worktree=True)
        _run_build_in_tmp(tmp_path, config=config, cli_options={"skip_manifest_check": True})
    ...
    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_cli_worktree_overrides_config(self, mock_which, mock_call, tmp_path: Path) -> None:
        config = _make_config(worktree=False)
        _run_build_in_tmp(
            tmp_path,
            config=config,
    ...
    @mock.patch.object(subprocess, "call", return_value=0)
    @mock.patch.object(shutil, "which", return_value="/usr/local/bin/ralphex")
    def test_custom_prompts_dir(self, mock_which, mock_call, tmp_path: Path) -> None:
        custom_prompts = tmp_path / "custom" / "prompts"
        custom_prompts.mkdir(parents=True)
        (custom_prompts / "custom_task.txt").write_text("custom content")
