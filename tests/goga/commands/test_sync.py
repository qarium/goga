...
class TestSyncLocal:
    ...
    def test_sync_local_os_error_during_copy(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with mock.patch("goga.commands.sync.shutil.copytree", side_effect=OSError("Permission denied")):
            runner = CliRunner()
            result = runner.invoke(sync, [str(source)])
...
class TestSyncGit:
    def test_sync_git_https_clones_and_syncs(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            mock.patch("goga.commands.sync.tempfile.mkdtemp", side_effect=fake_mkdtemp),
            mock.patch("goga.commands.sync.subprocess.run", side_effect=fake_run),
            mock.patch("goga.commands.sync.shutil.rmtree"),
        ):
            runner = CliRunner()
    ...
    def test_sync_git_not_installed(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("goga.commands.sync.tempfile.mkdtemp", side_effect=fake_mkdtemp),
            mock.patch(
                "goga.commands.sync.subprocess.run",
    ...
    def test_sync_tmp_dir_cleaned_up_on_success(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            mock.patch("goga.commands.sync.tempfile.mkdtemp", side_effect=fake_mkdtemp),
            mock.patch("goga.commands.sync.subprocess.run", side_effect=fake_run),
            mock.patch("goga.commands.sync.shutil.rmtree") as mock_rmtree,
        ):
            runner = CliRunner()
            runner.invoke(sync, ["https://github.com/user/repo.git"])
