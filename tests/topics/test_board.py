...
- ``BoardRecord(topic, branch, statuses, current, remote)`` — one row of the
  topic board, a topic hosted by one branch
...
def _wire_board(
    ...
    """Patch the board's import points: scale, git inventory, trees, branch."""
...
def _rows(records: list[BoardRecord]) -> list[tuple[str, str, list[str], bool, bool]]:
    """The records as plain tuples — topic, branch, statuses, current, remote."""
    return [
        (record.topic, record.branch, record.statuses, record.current, record.remote)
...
class TestCollectTopicBoard:
    def test_collect_topic_board_local_collapses_twin_and_marks_current(
        ...
        assert _rows(records) == [
            ("feat-b", "origin/feat/b", ["defined"], False, True),
            ("feat-a", "feat/a", ["planned"], True, False),
        ]
    ...
    def test_collect_topic_board_remote_mode_twin_current(
        ...
        assert _rows(records) == [
            ("feat-b", "origin/feat/b", ["defined"], False, True),
            ("feat-a", "origin/feat/a", ["planned"], True, True),
        ]
    ...
    def test_current_branch_empty_slug_hosts_no_topic(
        ...
        assert _rows(records) == [("feat-a", "feat/a", ["planned"], False, False)]
    ...
    def test_collect_topic_board_no_current_branch(
        ...
        assert _rows(records) == [
            ("feat-b", "origin/feat/b", ["defined"], False, True),
            ("feat-a", "feat/a", ["planned"], False, False),
        ]
    ...
    def test_board_sees_only_committed_artifacts_on_other_refs(
        ...
        # is visible.
        assert local_rows == [("feat-a", "feat/a", ["planned"], True, False)]
        # The same work through its remote twin reads the ref tree — the
        # uncommitted artifact is invisible there.
        assert remote_rows == [("feat-a", "origin/feat/a", ["empty"], True, True)]
