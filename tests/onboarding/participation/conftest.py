"""Shared fixtures of the participation cell tests — the environment boundary.

The delivery reaches the outside world through the hooks platform at exactly
two points: the installed-distributions mapping read by
``packages_distributions`` and the ``sys.modules`` entry of a ``goga_tool_*``
package. The fixtures below mirror the ``tests/hooks/conftest.py`` boundary
pins — conftest fixtures do not cross test directories — so the participation
mediator, the registry, and the package access run for real.
"""
...
def pin_package_environment(
    ...
    top-level module name mapped to the distributions providing it. Only the
    keys matter to the platform: the filter keeps the ``goga_tool_``-prefixed
    names. Pinning the mapping is what keeps the real installed tool packages
    of the development environment out of the enumeration.
