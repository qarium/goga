"""Shared fixtures of the generator cell tests — the environment boundary.

The cross-entity negative trace drives the participation delivery for real:
the installed-distributions mapping read by ``packages_distributions`` and the
``sys.modules`` entry of a ``goga_tool_*`` package are the only outside points.
The fixtures below mirror the ``tests/hooks/conftest.py`` boundary pins —
conftest fixtures do not cross test directories — so the mediator, the
registry, and the generator run for real.
"""
...
def pin_package_environment(
    ...
    top-level module name mapped to the distributions providing it. Pinning
    the mapping is what keeps the real installed tool packages of the
    development environment out of the enumeration.
