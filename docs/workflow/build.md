...
`goga build` runs a deterministic pipeline. Host-side steps (1–10) assemble the environment and launch the container; in-container steps (11–12) prepare the ralphex configuration and delegate the launch to `run_ralphex` (goga/ralphex), which invokes the external `ralphex` binary on `$PATH` and propagates its exit code.
...
| 4. Project preconditions | host | Resolve proxy (CLI `--proxy` wins over `config.build.proxy`); resolve hosts (CLI `--add-host` merges on top of `config.build.hosts`, CLI wins on conflict); when `--skip-manifest-check` is not set, scan `git status` for uncommitted `CODEMANIFEST` files and reject with exit 1 if any are found. |
...
| 10. Docker launch | host | Launch ralphex inside the configured Docker image with `python -m goga.build` as the entry point. A SIGTERM/SIGINT handler is installed before the secret env-file is written so a signal during setup unwinds to `finally` and unlinks the file. |
| 11. Docker guard | in-container | `main()` calls `ensure_in_docker()` as its very first statement — refuse to proceed outside the goga Docker image. |
