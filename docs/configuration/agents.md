...
The value is stripped of surrounding whitespace before resolution; empty / whitespace-only values are rejected up front with a `ValueError`. After stripping, the value is forwarded verbatim — case-folding and other normalization do not happen.

Edge cases:

| Edge case                                                                          | What happens                                                                                                                            | Where it surfaces                                                                                                                 |
|------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------|
| `agent: ""` / whitespace-only                                                      | Rejected up front with `ValueError` (`build.task_executor.agent is required` / `pipeline.agent is required`).                           | At config load — `goga config`, `goga build`, `goga pipeline` exit non-zero before any container starts.                          |
