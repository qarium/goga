...
Each action in the catalog fixes how a failing hook is treated. The topic-status, the onboarding, the topics, the two pipeline notification, the four build notification, and the four usages notification actions are **soft**: a failing hook is skipped with a stderr warning naming the tool, the action, and the reason, and the command continues. A **hard** action stops the command — `pipeline/amend_workflow` stops at the first failing hook with a clean error (see [Pipelines — Hooks](../pipelines/hooks.md)), `build/validate_build` lets every subscribed tool's hooks run to completion and merges their vetoes into one error that stops the build before any pass (see [Build — Hooks](../build/hooks.md)), and `config/amend_config` stops at the first failing tool — a crashed hook or a structurally malformed contribution, the tool's whole contribution discarded — with a clean error naming the tool and the action (see [Configuration — Hooks](../../configuration/hooks.md)), and `schema/amend_cell` stops at the first failing tool the same way, its clean error naming the tool, the action, and the failing cell path (see [Schema — Hooks](../schema/hooks.md)); the class is chosen by the owner domain when it declares the action.
