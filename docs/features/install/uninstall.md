...
- `goga uninstall` never reads or writes `connect.yml` itself — [`goga connect`](../connect/cli.md) is the single writer of the registry, reached only through the shared re-sync routine.
