...
- [x] Создать/обновить файл `tests/goga/commands/test_integration_split.py`
- [x] Протестировать: sync CLI вызывает sync_logic с правильными аргументами и пробрасывает exit_code
- [x] Протестировать: schema CLI вызывает schema_logic, обрабатывает ValueError (exit 1), выводит JSON через click.echo
- [x] Протестировать: build CLI вызывает build_logic с config и cli_options, пробрасывает exit_code
- [x] Протестировать: install CLI вызывает install_logic с agent и config, пробрасывает exit_code
- [x] Запустить валидацию: `python -m pytest tests/ -v`
...
- [x] Каждая сущность контракта доступна из фасада (goga.sync, goga.schema, goga.build, goga.install)
- [x] CLI-обёртки в goga.commands делегируют бизнес-логику через `build_logic`, `install_logic`, `schema_logic`, `sync_logic`
- [x] Описания отражены в поведении (exit codes, error handling, форматы вывода)
- [x] Зависимости контракта соблюдены (Imports из CODEMANIFEST)
- [x] Реэкспорты доступны из фасада
...
- [x] Интеграционные тесты существуют для межсущностных сценариев
